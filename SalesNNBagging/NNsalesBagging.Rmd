...
**ASSIGNMENT:**    
  1. Find the 'best' sales target price for predicting sales
    ...
    -I determined the best sales price by looking at the median sales price of the cars sold. The median for the data set was 9130, so that is why I set my sales price at $9130.
    
  2. Determing values of age, gender, income level or commute distance *i.e.* miles/wk that predict best sales
  
    - I used the logistic regression to determine the values to use in the Neural Network. I used the plot of probability of sales versus each of the variables, except gender, to determine my initial marketing strategy.
    
  3. Determing the appropriate neural net - how many hidden layers, what variables to leave out?  
  
    - I left out gender as a result of the linear regression. I determined to use 2 hidden layers, as it allowed for quicker processing while still keeping about a 50% accuracy, which was similar to running 3 layers.
  
  4. Does it help to use 80/20 (or any other) split for training/test sets? 
  
    -I used a 75% for training and 25% for testing.  
  
  5. Recommend an advetising approach based on your findings  
  
    - As a result of the neural network, I would choose to advertise to anyone younger than 40 years onld with an income greater than $4500. The advertising strategy would be targeted to lower mileage users especialy under 40 miles per day and people with less than $5000.
...
averageAge = mean(testDataSet$age)
# logistic regresson coefficients from previous chunk

xage = b[1]+b[2]*testDataSet$age + b[3]*averageMiles +
  b[4]*averageDebt + b[5]*averageIncome
pred.prob = 1./(1+exp(xage))
plot(testDataSet$age, pred.prob, ylab='prob of sale', xlab='age')
grid()

xmiles = b[1]+b[2]*averageAge + b[3]*testDataSet$miles +
  b[4]*averageDebt + b[5]*averageIncome
pred.prob = 1./(1+exp(xmiles))
plot(testDataSet$miles, pred.prob, ylab='prob of sale', xlab='miles')
grid()

xincome = b[1]+b[2]*averageAge + b[3]*averageMiles +
  b[4]*averageDebt + b[5]*testDataSet$income
pred.prob = 1./(1+exp(xincome))
plot(testDataSet$income, pred.prob, ylab='prob of sale', xlab='income')
grid()

xdebt = b[1]+b[2]*averageAge + b[3]*averageMiles +
  b[4]*testDataSet$debt + b[5]*averageIncome
pred.prob = 1./(1+exp(xdebt))
plot(testDataSet$debt, pred.prob, ylab='prob of sale', xlab='debt')
grid()
...
salesNet = neuralnet(salesTgt~age+miles+debt+income, data = normalizedTrainDataSet, hidden=c(2,2), stepmax=1.e6, rep=1, linear.output = TRUE)
```
plot..
...
x = filter(cars, age < 40 & income>4500 & debt < 10000 & miles < 40)
...
for (i in 1:numberBags) {
  ...
  predX = predict(NN, x)
  vectorOfTables[[i]]=table(predX[,1]>0.2, x$salesTgt)
}
